...
{
    public abstract class QueryBase<TEntity> : IQuery<TEntity> where TEntity : class, IEntity<int>, new()
    {
        ...
        public string SortAccordingTo
        {
            ...
            protected set
            {
                var properties = typeof(TEntity).GetProperties().Select(prop => prop.Name).Except(new[] {nameof(TEntity)}); // ToDo maybe fix this later
