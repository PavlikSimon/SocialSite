...
namespace QueryInfrastructureEntityFramework
{
    public class EntityFrameworkQuery<TEntity> : QueryInfrastracture.QueryBase<TEntity> where TEntity : class, IEntity<int>, new()
    {
        ...
        protected DbContext Context => ((EntityFrameworkUnitOfWork)Provider.GetCurrent()).Context;
        ...
        public override async Task<QueryResult<TEntity>> ExecuteAsync()
        {
            ...
            if (string.IsNullOrWhiteSpace(SortAccordingTo) && DesiredPage.HasValue)
            {
                ...
                SortAccordingTo = nameof(IEntity<int>.Id);
